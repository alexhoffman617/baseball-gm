...
export class PlayoffScheduleGenerator {
  ...
  async generatePlayoffSchedule() {
    ...
    const teams = []
    _.each(this.sharedFunctionsService.getRecordOrderedTeams().splice(0, 4), function(team){
      teams.push(team._id)
    })
    const schedule = []
    const round1 = []
    for (let x = 0; x < teams.length / 2; x ++) {
      round1.push(new PlayoffMatchup(teams[x], teams[teams.length - 1 - x], 7))
    }
  ...
  async generateNextRound(playoffSchedule) {
    ...
    this.leagueDataService.updateLeague()
    const nextRound = []
    for (let x = 0; x < playoffSchedule[playoffSchedule.length - 1].length / 2; x++) {
      nextRound.push(
        new PlayoffMatchup(
          this.getPlayoffMatchupWinner(playoffSchedule[playoffSchedule.length - 1][x]),
          this.getPlayoffMatchupWinner(playoffSchedule[playoffSchedule.length - 1]
            [playoffSchedule[playoffSchedule.length - 1].length - 1 - x]), 7))
    }
